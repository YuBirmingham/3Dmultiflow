...
!##########################################################################
	  ...
	  character (LEN=6) :: name_end
	  character (LEN=4) :: dominio
     ...
     &			TRIM(ADJUSTL(name_end)) 
			write(dominio,'(I4)') (dom_id(ib)-nimapp)  !Identification of targetted domain
        		strlen=LEN(TRIM(ADJUSTL(dominio)))
       		dominio=REPEAT('0',(4-strlen))//
     &			TRIM(ADJUSTL(dominio)) 

			filename='inflow/Inlet_'//dominio//'_'//name_end//'.dat'
...
!##########################################################################
     ...
     &        dom(ib)%xc(i+1).ge.xmapp) then
		...
		write(dominio,'(I4)') (dom_id(ib)-nimapp)
  		strlen=LEN(TRIM(ADJUSTL(dominio)))
 		dominio=REPEAT('0',(4-strlen))//
     &	TRIM(ADJUSTL(dominio)) ! e.g. "0001"
	       filename='inflow/Mapping_'//dominio//'.dat'
      	open (unit=ireadinlet+500, file=filename)
